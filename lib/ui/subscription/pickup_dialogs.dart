...
Future<void> deletePickupDialog({BuildContext context, Function onDeletePickup, Pickup pickup}) async {
  return showDialog(
    ...
    barrierDismissible: false,
    builder: (BuildContext context) {
      final String date = weekdayAndDate(context, pickup.pickupDate);
      return AlertDialog(
        title: Text('Supprimer le passage ?'),
        content: Text('Le passage du $date sera supprimé, et un nouveau passage sera ajouté à la fin de votre abonnement'),
        actions:  <Widget>[
          FlatButton(
            child: Text('ANNULER'),
            onPressed: () {
