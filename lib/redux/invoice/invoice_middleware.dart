...
class InvoiceMiddleware {
  ...
  const InvoiceMiddleware(this.repository);
  ...
  void _loadInvoicesRequest(
      Store<AppState> store, LoadInvoicesRequest action, NextDispatcher next) async {
    next(action);

    try {
      final List<dynamic> data = await repository.fetchInvoices(action.customer.id);
      final List<Invoice> invoices =
          List<Invoice>.from(data.map<dynamic>((dynamic x) => Invoice.fromJson(x)));
  ...
  void _loadInvoiceItemsRequest(
      Store<AppState> store, LoadInvoiceItemsRequest action, NextDispatcher next) async {
    next(action);

    try {
      final List<dynamic> data = await repository.fetchInvoiceItems(action.customer.id);
      final List<InvoiceItem> invoiceItems =
          List<InvoiceItem>.from(data.map<dynamic>((dynamic x) => InvoiceItem.fromJson(x)));
  ...
  void _createInvoiceItemRequest(
      Store<AppState> store, CreateInvoiceItemRequest action, NextDispatcher next) async {
    next(action);
